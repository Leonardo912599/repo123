...
public class ControladorAgendarCita implements MouseListener, ActionListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == vistaAgendarCita.button_agendarCita) {
            ...
            vac.txt_nombreMascota.setText(vistaAgendarCita.table_cita.getValueAt(row, 7).toString());
            vac.txt_animal.setText(vistaAgendarCita.table_cita.getValueAt(row, 6).toString());
            vac.txt_sexo.setText(vistaAgendarCita.table_cita.getValueAt(row, 8).toString());
            vac.txt_raza.setText(vistaAgendarCita.table_cita.getValueAt(row, 9).toString());
            ...
            Cliente cliente = mascotaDao.obtenerClienteDeMascota(vistaAgendarCita.table_cita.getValueAt(row, 7).toString());
    ...
    public void listarCitas() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            row[6] = list.get(i).getAnimal().getNombre();
            row[7] = list.get(i).getMascota().getNombre();
            row[8] = list.get(i).getMascota().getSexo();
            row[9] = list.get(i).getMascota().getRaza();
